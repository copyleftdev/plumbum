    ...
    F: FnMut(DnsRecord),
{
    ...
    let mut pcap = PcapReader { reader, big_endian, nanosecond, link_type: 0 };
    ...
    if pcap.link_type != 1 && pcap.link_type != 113 {
        return Err(io::Error::new(
            ...
            format!("Unsupported link type: {} (only Ethernet/1 and Linux SLL/113 supported)", pcap.link_type),
        ));
    }
    ...
    F: FnMut(DnsRecord),
{
    ...
    let remaining = if shb_len > 12 { shb_len - 12 } else { 0 };
    let mut skip_buf = vec![0u8; remaining];
    reader.read_exact(&mut skip_buf)?;

    let read_u32 = |buf: &[u8], be: bool| -> u32 {
        if be { u32::from_be_bytes([buf[0], buf[1], buf[2], buf[3]]) }
        else { u32::from_le_bytes([buf[0], buf[1], buf[2], buf[3]]) }
    };
    let read_u16 = |buf: &[u8], be: bool| -> u16 {
        if be { u16::from_be_bytes([buf[0], buf[1]]) }
        else { u16::from_le_bytes([buf[0], buf[1]]) }
    };
    ...
    loop {
        ...
        let body_len = if block_len >= 12 { block_len - 12 } else { 0 };
            ...
            PCAPNG_IDB => {
                if body.len() >= 4 {
                    ...
                    while opt_off + 4 <= body.len() {
                        ...
                        let opt_len = read_u16(&body[opt_off + 2..opt_off + 4], big_endian) as usize;
                        opt_off += 4;
                        if opt_code == 0 { break; }
...
fn parse_packet(data: &[u8], timestamp: f64) -> Option<DnsRecord> {
    if data.len() < ETH_HEADER_LEN { return None; }
...
fn parse_sll_packet(data: &[u8], timestamp: f64) -> Option<DnsRecord> {
    if data.len() < SLL_HEADER_LEN { return None; }
    let protocol = u16::from_be_bytes([data[14], data[15]]);
    if protocol != 0x0800 { return None; }
...
fn parse_ipv4_dns(data: &[u8], timestamp: f64) -> Option<DnsRecord> {
    ...
    if ihl < 20 || data.len() < ihl { return None; }
    let protocol = data[9];
    ...
    if udp_data.len() < UDP_HEADER_LEN { return None; }
...
fn parse_dns(
    data: &[u8], timestamp: f64,
    src_ip: String, src_port: u16, dst_ip: String, dst_port: u16,
) -> Option<DnsRecord> {
    if data.len() < DNS_HEADER_LEN { return None; }
    ...
    for i in 0..qd_count {
        ...
        offset += 4;
        if i == 0 { query_name = name; query_type = qtype; }
    }
    ...
    Some(DnsRecord {
        ...
        proto: "udp".to_string(),
        trans_id, query_name, query_type,
...
fn read_dns_name(data: &[u8], start: usize) -> Option<(String, usize)> {
    ...
    loop {
        ...
        if (len_byte & 0xC0) == 0xC0 {
            ...
            if !jumped { return_offset = offset + 2; }
            jumped = true;
            offset = pointer;
        } else {
...
fn decode_rdata(rtype: u16, rdata: &[u8], full_packet: &[u8]) -> String {
    match rtype {
        1 => {
            ...
            } else { hex_encode(rdata) }
        }
        28 => {
            if rdata.len() == 16 {
                (0..8).map(|i| format!("{:x}", u16::from_be_bytes([rdata[i*2], rdata[i*2+1]])))
                    .collect::<Vec<_>>().join(":")
            } else { hex_encode(rdata) }
        }
        2 | 5 | 12 => {
            let off = offset_in_packet(rdata, full_packet);
            if let Some((name, _)) = read_dns_name(full_packet, off) { name }
            else { hex_encode(rdata) }
        }
        15 => {
            if rdata.len() >= 2 {
...
fn decode_txt_rdata(rdata: &[u8]) -> String {
    ...
    while offset < rdata.len() {
        ...
        offset += 1;
        if offset + str_len > rdata.len() { break; }
